...
object Server extends ServerModuleInterface:
  ...
  val reframedFlow: Flow[ByteString, String, NotUsed] =
    Framing
      .delimiter(ByteString("\n"), allowTruncation = false, maximumFrameLength = 256)
      .map(_.utf8String)
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] =
    reframedFlow.map(Identity.parse(_)).toMat(Sink.head)(Keep.right)
  /**
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () => {
      ...
      var next = 1
      { case e if (e.sequenceNr == next && buffer.isEmpty) => 
          next += 1
          List(e)

        case e if (e.sequenceNr == next && !buffer.isEmpty) =>
          buffer = e :: buffer
          val cBuffer = buffer.sortBy(_.sequenceNr)
          val newBuffer = buffer.dropWhile(ev => ev.sequenceNr <= next+1)
          val result = cBuffer.takeWhile(ev => ev.sequenceNr <= next+1)
          buffer = newBuffer
          next = result.last.sequenceNr + 1
          result
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat { () => 
      var states = Map.empty[Int, Set[Int]]
      { case e @ Follow(seq, from, to) => 
          states = states + (from -> (states.getOrElse(from, Set.empty) + to))
          List((e, states))
        case e @ Unfollow(seq, from, to) =>
          states = states + (from -> (states.getOrElse(from, Set.empty) - to))
          List((e, states))
        case e @ _ => List((e, states)) 
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean = eventAndFollowers match 
      case (Event.Follow(_, _, to), _) => 
        to == userId

      case (Event.Unfollow(_, _, _), _) =>
        false

      case (Event.Broadcast(_), _) => 
        true

      case (Event.PrivateMsg(_, _, to), _) => 
        to == userId 

      case (Event.StatusUpdate(_, from), f) =>
