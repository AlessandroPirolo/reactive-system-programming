...
object SelectiveReceive:
  ...
  private def intercept[T: ClassTag](bufferSize: Int, buffer: StashBuffer[T], started: Behavior[T]): Behavior[T] =
    Behaviors.receive { case (ctx, message) =>
      ...
      if (Behavior.isUnhandled(next) == true) {
        if (buffer.size == bufferSize) {
          throw StashOverflowException("buffer size exceeded!")
        } else {
          buffer.stash(message)
          Behaviors.same
        }
      } else {
        val canonicalize = Behavior.canonicalize(next, started, ctx)
        buffer.unstashAll(SelectiveReceive(bufferSize, canonicalize))
      }
    }
