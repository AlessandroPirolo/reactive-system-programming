  ...
  private def idle[T](value: T, sessionTimeout: FiniteDuration): Behavior[PrivateCommand[T]] =
    Behaviors.receivePartial {
      case (ctx, Begin(replyTo)) =>
        val anonymous = ctx.spawnAnonymous(sessionHandler(value, ctx.self, Set.empty))
        ctx.setReceiveTimeout(sessionTimeout, RolledBack(anonymous))
        ctx.watchWith(anonymous, RolledBack(anonymous))
        replyTo ! anonymous
        inSession(value, sessionTimeout, anonymous)
  ...
  private def sessionHandler[T](currentValue: T, commit: ActorRef[Committed[T]], done: Set[Long]): Behavior[Session[T]] =
      Behaviors.receive[Session[T]] {
        ...
        case (ctx, Modify(f, id, reply, replyTo)) =>
          ...
          sessionHandler(result, commit, ndone)
        case (ctx, Rollback()) =>
          Behaviors.stopped
